...
4. Carry the existing operations salary into all three columns. The reference holds labour at 160 hours a month on the same salary: cloud operations engineers transition to on-prem platform work, with contracted physical tasks handled by colocation remote hands or the rental provider and costed in the facility or rental line. On-prem roles can cost less than cloud specialist roles; this conservative model counts none of that potential saving. Validate hours and staffing costs in the pilot, then budget measured recurring differences. Charge setup, training and migration once.
