...
- **Google Cloud:** Google Cloud Pricing Calculator — billing-account pricing requires sign-in; sustained-use discounts depend on the machine series and do not stack with committed-use discounts.
- **Azure:** Azure Pricing Calculator — negotiated rates require sign-in and the matching billing agreement; both Enterprise Agreement and Microsoft Customer Agreement price sheets are supported.
...
3. Fill the colocation column from hardware and facility quotes: sixteen machines and two spares over sixty months, rack, power, transit, cross-connects and remote hands. Quote equivalent rented metal with traffic and hardware support. Match availability and recovery objectives: a single rack cannot replace independent cloud zones without changing that promise. Add any second site, VM licences, hypervisor support and off-site backup capacity the estate requires.
4. Carry the existing operations salary into all three columns. The reference assumes 160 hours a month on the same salary: cloud operations engineers take on platform work, with contracted physical tasks priced in remote hands or the rental line. Validate recurring hours, skills and on-call cover in the pilot, then budget measured differences. Charge setup, training and migration once. The published figures describe a container reference estate; price the actual VM mix separately.
...
## Operator's notes
- **Swap:** Price the three largest workloads for an initial screen, then include retained services, shared costs and labour before approving a budget.
